...
data <- read_mpath(# reads data
  file = "example_dataset.csv", # path to the data (replace with own data if desired)
  meta_data = "meta.csv" # path to the metadata
) 
```
...
```{r print data info}
dataInfo(file_path="../read_mpath/example_dataset.csv", 
         read_fun = read.csv2,
...
The time stamps from m-Path are in [Unix time](https://en.wikipedia.org/wiki/Unix_time). To work with them, we will reformat them to a more readable format. We will use the `as.POSIXct()` function from the `lubridate` package to convert the time stamps to a POSIXct object. We will also specify the following format: `"%Y-%m-%d %H:%M:%OS"`. We are specifying the timezone as UTC.

```{r reformat time stamps}
data <- data %>%
  mutate(across(
    c(scheduled, sent, start, stop), # we specify the columns we want to reformat
    \(x) as.POSIXct(x, format="%Y-%m-%d %H:%M:%OS", tz = 'UTC',origin = '1970-01-01 00:00:00') # we specify the time format
  ))
```
...
```{r compliance rate}
source('../response_rate_functions/response_rate_functions.R') # just manually loading the response_rate_functions until the package is done
