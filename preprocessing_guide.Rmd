...
data <- read_mpath(# reads data
  file = "../read_mpath/example_dataset.csv", # path to the data (replace with own data if desired)
  meta_data = "../read_mpath/meta.csv" # path to the metadata
) 
```
...
data <- data %>%
  mutate(across(
    ...
    \(x) as.POSIXct(x, format="%Y-%m-%d %H:%M:%OS", tz = 'UTC') # we specify the time format
  ))
```
